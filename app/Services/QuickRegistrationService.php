...
final class QuickRegistrationService
{
    ...
    public function availability(Tournament $tournament): array
    {
        ...
        $message = match (true) {
            ...
            $tournament->status !== TournamentStatus::Registration => 'El torneo no está recibiendo inscripciones.',
            $tournament->registration_starts_at?->isFuture() => 'El periodo de inscripción todavía no inicia.',
            $tournament->registration_ends_at?->isPast() => 'El periodo de inscripción ya finalizó.',
            $tournament->draw()->exists() || $tournament->groups()->exists() => 'Las inscripciones ya fueron cerradas por el organizador.',
