...
@section('content')
    ...
    <div class="row g-4 mb-4">
        ...
        <div class="col-lg-7">
            <div class="mp-card p-4 h-100">
                <div class="d-flex justify-content-between gap-2"><h2 class="h5 fw-bold">Primera ronda</h2><span class="mp-muted">{{ $plan['bracket_size'] }} espacios · {{ $plan['bye_count'] }} byes</span></div>
                    ...
                    @foreach ($plan['pairs'] as $index => $pair)
                        <div class="col-md-6">
                            <div class="mp-match-card w-100">
                                ...
                                <div class="mp-match-participant"><span>{{ $pair['participant_b'] ? ($tournament->participant_type === App\Enums\ParticipantType::Individual ? $pair['participant_b']->nickname : $pair['participant_b']->name) : 'Pase automático' }}</span></div>
                            </div>
                        </div>
                    @endforeach
                </div>
    ...
    <form method="post" action="{{ route('tournaments.draws.store', $tournament) }}" data-confirm="¿Confirmar este sorteo? Las inscripciones quedarán bloqueadas.">
        ...
        <button class="btn btn-primary">Confirmar y generar primera ronda</button>
    </form>
@endsection
